...
## 项目定位
ChunkPilot 是 **RAG 知识库管理后台**，不是问答产品。LLM 唯一用途 = 判定切分/嵌入参数（不做对话）。真正问答由外部 Agent（hermes / claude code / opencode）经 **MCP**（后端 `/mcp`，Streamable HTTP）连进来自行完成。
...
- api_key 仅存后端进程内，**不回传明文、不写日志**，前端只显示掩码。
